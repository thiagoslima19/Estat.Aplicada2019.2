...
dados1$RACACORMAE=factor(dados1$RACACORMAE,levels=c("Branca","Negra","Amarela","Parda","Indígena"))
...
dados1$PARTO=factor(dados1$PARTO,labels=c("Vaginal","Cesáreo"))
...
barplot(prop.table(table(GRAVIDEZ2)),main="Tipos de Gravidez",ylab="Frequência",xlab="Tipos",col=mypalette[c(2,6)])
#c)
dados1$IDADECAT=cut(IDADEMAE,breaks=c(17,20,34,40))
attach(dados1)
ee=filter(dados1,PARTO=="Cesário")
...
hist(ee$IDADEMAE,breaks=c(17,20,34,40),xlim=c(15,40),freq=T,plot=T,main="Número de Gestantes por Faixa Etária", xlab="Idades",ylab="Frequência",col=c("cadetblue","blue2","blue4"))

barplot(prop.table(table(IDADECAT)))
hist(SEMAGESTAC)
hist(IDADEMAE,breaks=c(17,20,25,29,33,37,40),xlim=c(15,40),freq=T,plot=T,main="Número de Gestantes por Faixa Etária", xlab="Idades",ylab="Frequência",col=mypalette[8])
lines(density(IDADEMAE),col="yellow")
barplot(prop.table(table(GRAVIDEZ2)),main="Tipos de Gestação",xlab="Tipos",ylab="Frequência",col=c("Cyan","blue"))      
civ=ESTCIVMAE
...
barplot(prop.table(table(civ)),main="Estado Civil das Gestantes",ylim=c(0,0.6),col=c(mypalette[c(4,7)]),ylab = "Frequências")
summary(civ)
hist(PESO,main="Distribuição do Peso dos Bebês",col="blue",ylab="Frequência",xlab="Peso dos Bebês")
...
hist(PESO[RACACOR2=="Branca"],main="Distribuição do Peso dos Bebês Brancos",col="blue4",ylab="Frequência",xlab="Peso dos Bebês")
hist(PESO[RACACOR2=="Negra"],main="Distribuição do Peso dos Bebês Negros",col="cyan",ylab="Frequência",xlab="Peso dos Bebês")
hist(PESO[RACACOR2=="Outros"],main="Distribuição do Peso dos Bebês Amarelos,Pardos ou Indígenas",col="cadetblue",ylab="Frequência",xlab="Peso dos Bebês")
par(mfrow=c(1,1))
...
barplot(t, beside=TRUE, col=mypalette[c(2,9)],main="Número de Gestantes por Escolaridade e Estado Civil",ylab="Frequência")
...
ggplot(rr, aes(x=ESTCIVMAE,y=IDADEMAE,fill=)) + 
  geom_boxplot()+scale_fill_manual(values=c(mypalette[c(4,7)]))+ggtitle("Tipos de Partos em relação a Idade das Gestantes")+xlab("Tipos de Parto")+ylab("Idade")
tt=select(dados1,RACACOR2,PESO)
ggplot(tt, aes(x=RACACOR2,y=PESO,fill=RACACOR2)) + 
  geom_boxplot()+scale_fill_manual(values=c(mypalette[c(1,6,7)]))+ggtitle("Tipos de Partos em relação a Idade das Gestantes")+xlab("Parto")+ylab("Idade")
summary(dados1)
t=prop.table(table(PARIDADE,KOTELCHUCK))
barplot(t, beside=TRUE, col=mypalette[c(1,6)],main="Relação de Paridade e Índice de Kotelchuck",xlab="Índice de Kotelchuck",ylab="Frequência")
legend(locator(1), xpd=TRUE, ncol=2, legend=c("1ªGestação","Não é 1ªGestação"),
...
barplot(o,beside=T, col=mypalette[c(4,9)],main="Número de Gestantes por Escolaridade e Estado Civil",ylab="Frequência",xlab="Escolaridade")
legend(locator(1), xpd=TRUE, ncol=2, legend=c("Sem companheiro","Com companheiro"),
